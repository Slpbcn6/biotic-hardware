...
- **`run.py`**: `pipeline_version` and `seeds` read from `parameters.json`
  (`VIII_pipeline.version`, `VI_experimental_sweep_parameters.multi_seed_list`) instead of being hardcoded; the stale `SEEDS` import from `multi_seed_analysis.py` is removed. The pipeline header version string and seed range are now dynamic.
